...
local ToCamelCase = function(text)
...
if not isfile("Unbounded Yield v2/configs/globalConfigs.bin") then
	writefile("Unbounded Yield v2/configs/globalConfigs.bin", "true")
end

local isGlobalConfigOn = readfile("Unbounded Yield v2/configs/globalConfigs.bin") == "true"
...
local library = {
	...
	tabSize = 0,
	theme = {},
	foldername = isGlobalConfigOn and "Unbounded Yield v2/configs/global" or string.format("Unbounded Yield v2/configs/%s", tostring(lplr.UserId)),
	fileext = ".json",
...
do
    ...
    local onInputBegan = function(input, gpe)
    ...
    local makeTooltip = function(interest, option)
	...
	function library:Unload()
		...
		for _, option in next, self.options do
			if option.type == "toggle" and not string.find(string.lower(option.flag), "panic") and option.flag ~= "saveconfigauto" then
				pcall(option.SetState, option, false)
			end
		end

		for _, connection in pairs(library.scriptConnections) do
    ...
    local readFileAndDecodeIt = function(filePath)
    ...
    local getConfigForGame = function(configData) -- not used
    ...
    function library:LoadConfig(configName)
        ...
        if not configData then DEBUG_PRINT("no config found", configName) return end
    ...
    local createLabel = function(option, parent)
    ...
    local createDivider = function(option, parent)
    ...
    local createToggle = function(option, parent)
        option.hasInit = true
    ...
    local createButton = function(option, parent)
        option.hasInit = true
    ...
    local createBind = function(option, parent)
        option.hasInit = true
        ...
        local formatKeys = function(keys)
        ...
        local isKeybindPressed = function()
            local foundCount = 0
    ...
    local createSlider = function(option, parent)
        option.hasInit = true
    ...
    local createList = function(option, parent)
        option.hasInit = true
        ...
        local updateHolder = function(newValueCount)
    ...
    local createBox = function(option, parent)
        option.hasInit = true
    ...
    local createColorPickerWindow = function(option)
    ...
    local createColor = function(option, parent)
        option.hasInit = true
    ...
    function library:Init(silent)
        ...
        local clearFoundResult = function()
        ...
        if configData then
            ...
            DEBUG_PRINT("something went wrong with loading ui lib (2)", configData)
        end
    ...
    do
        ...
        local showBasePrompt = function(text)
        ...
        local settingsTab = library:AddTab("⚙️", 100)
        ...
        task.spawn(function() -- // this just makes it load faster
        ...
        settingsMenu:AddToggle({
            ...
            callback = function(val)
                return visualizer:SetEnabled(val)
            end
        }):AddColor({
            text = "Keybind Visualizer Color",
            callback = function(val)
                return visualizer:UpdateColor(val)
            end
        })
        ...
        settingsMenu:AddList({
            ...
            callback = function(val)
                if Backgrounds[Value] then
                    library.main.Image = "rbxassetid://" .. Backgrounds[val]
                end
            end
        }):AddColor({
        ...
        local getAllConfigs = function()
            ...
            for _, v in next, listfiles("Unbounded Yield v2/configs") do
        ...
        local updateAllConfigs = function()
        ...
        configSection:AddButton({
            ...
            callback = function()
                ...
                local configData = readfile(string.format("Unbounded Yield v2/configs/%s/%s", folderName, fullConfigName))
            ...
            callback = function(toggle)
				if toggle then
					repeat
						library:UpdateConfig()
                        task.wait(60)
					until not library.flags.saveConfigAuto
				else
        ...
        local saveConfigBeforeGameLeave = function()
        ...
        library.unloadMaid:GiveTask(lplr.OnTeleport:Connect(function(state)
...
do
	...
	library.mainTabs.visual = library:AddTab("Visual")
	library.mainTabs.movement = library:AddTab("Movement")
	library.mainTabs.utility = library:AddTab("Utility")

	library.mainColumns.combat1 = library.mainTabs.combat:AddColumn()
	library.mainColumns.combat2 = library.mainTabs.combat:AddColumn()
	library.mainColumns.visual1 = library.mainTabs.visual:AddColumn()
	library.mainColumns.visual2 = library.mainTabs.visual:AddColumn()
	library.mainColumns.movement1 = library.mainTabs.movement:AddColumn()
	library.mainColumns.movement2 = library.mainTabs.movement:AddColumn()
	library.mainColumns.utility1 = library.mainTabs.utility:AddColumn()
	library.mainColumns.utility2 = library.mainTabs.utility:AddColumn()
end

DEBUG_PRINT(string.format("ui lib Loaded in %.02f", tick() - startedLoadTick))
